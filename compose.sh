...
if [ -f .env ]; then
  ...
  if [ "$mode" = "prod" ]; then
    ...
    if docker compose version >/dev/null 2>&1; then
      ...
      docker compose $COMPOSE_ARGS --profile dev stop frontend 2>/dev/null || true
      # shellcheck disable=SC2086
      docker compose $COMPOSE_ARGS --profile dev rm -f frontend 2>/dev/null || true
    fi
  fi
fi
...
if docker compose version >/dev/null 2>&1; then
  # shellcheck disable=SC2086
  exec docker compose $COMPOSE_ARGS $COMPOSE_PROFILE "$@"
fi
