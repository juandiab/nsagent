...
def test_unknown_vendor_falls_back_to_netscaler_prompts():
    assert resolve_prompt_vendor("juniper") == "netscaler"
    assert prompts_dir("netscaler").is_dir()


def test_f5_operator_prompt_loads():
    prompt = load_role_prompt("operator", "f5")
    assert "F5 BIG-IP" in prompt
    assert "search_f5_tmsh_reference" in prompt
    assert "{{include:" not in prompt
