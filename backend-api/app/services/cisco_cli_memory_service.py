"""Load and search Cisco IOS switch memory for Copilot RAG."""
...
MEMORY_FILENAME = "cisco_ios_switch_memory.md"
MEMORY_VENDOR = "cisco"
...
def _load_memory_markdown() -> str:
    ...
    path = _memory_path()
    if not path.is_file():
        raise FileNotFoundError(f"Cisco memory file not found: {path}")
...
def _extract_commands(body: str) -> list[str]:
    ...
    for line in body.splitlines():
        ...
        if lowered.startswith(("show ", "display ", "configure ", "interface ", "copy ", "ping ", "traceroute ")):
            command = stripped.split("#", 1)[0].strip()
            key = command.lower()
            if key not in seen and len(command) <= 120:
                seen.add(key)
                commands.append(command)
    return commands


def search_cisco_cli_memory(
    query: str,
    max_sections: int = 3,
