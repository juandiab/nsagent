...
{"inputForm": {"title": "What are you planning?", "submitLabel": "Continue", "fields": [
  {"id": "planning_intent", "label": "Planning intent", "type": "choice", "required": true, "options": [
    {"value": "new_deployment", "label": "New appliance / greenfield deployment", "description": "New BIG-IP platform, site, or major rollout"},
    {"value": "new_functionality", "label": "New functionality on existing deployment", "description": "Add or change VS/pool/policy/iRule/APM on live infrastructure"},
...
Discovery focuses on **delta** — do **not** re-run full greenfield HA/platform discovery unless the user asks.

One topic per turn via ```jpilot-form```. Priority: existing TMOS/HA context, change scope, dependencies, impact, validation/rollback.
...
Before the final document, call `search_f5_documentation` for any vendor-specific maintenance/failover references if needed.

Deliverable: markdown **Change control record** with `<!-- jpilot-change-control-document -->` on the first line. ITIL/ServiceNow-style sections (vendor-neutral). Include **Vendor pre-change checklist** appendix for **F5 BIG-IP** (config-sync status, save sys config, UCS backup, HA failover readiness, QKView if needed, maintenance mode / force-offline plan).
