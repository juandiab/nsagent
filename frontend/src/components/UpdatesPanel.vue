    ...
    <div class="col-12 lg:col-8 flex flex-column gap-4">
      ...
      >
        ...
        <div class="command-block mt-4">
          ...
          <pre class="command-pre"><code>{{ linuxCommands }}</code></pre>
          <Button
            label="Copy"
            icon="pi pi-copy"
            size="small"
            ...
            outlined
            class="mt-2"
            @click="copyCommands(linuxCommands)"
          />
        </div>

        <div class="command-block mt-4">
          <div class="command-label">Windows (PowerShell)</div>
          <pre class="command-pre"><code>{{ windowsCommands }}</code></pre>
          <Button
            label="Copy"
            icon="pi pi-copy"
            size="small"
            ...
            outlined
            class="mt-2"
            @click="copyCommands(windowsCommands)"
          />
        </div>
...
const linuxCommands = computed(() =>
  (updateInfo.value?.update_instructions?.commands_linux_mac || []).join('\n')
)

const windowsCommands = computed(() =>
  (updateInfo.value?.update_instructions?.commands_windows || []).join('\n')
)
