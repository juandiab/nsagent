"""NetScaler SDX SVM SSH command validation and execution."""
...
def validate_readonly_command(command: str) -> str:
    ...
    if not normalized.lower().startswith(READONLY_PREFIX):
        raise ValueError("Only show commands are allowed via read-only SSH on SDX SVM")
    return normalized
...
def validate_writable_command(command: str) -> str:
    ...
    if lowered in {"shell", "reboot"} or lowered.startswith("bash "):
