"""F5 BIG-IP TMSH over SSH — command validation and execution."""
...
_SHELL_BREAKOUT = frozenset({"bash", "sh", "/bin/bash", "/bin/sh", "tclsh"})
_READONLY_BLOCKED = (
    " create ",
    " modify ",
    " delete ",
...
def _normalize_tmsh(command: str) -> str:
    normalized = normalize_cli_command(command)
    if not normalized:
        return normalized
    lowered = normalized.lower()
    if lowered.startswith(("show ", "list ", "create ", "modify ", "delete ", "save ")):
        return f"tmsh {normalized}"
    return normalized


def validate_readonly_command(command: str) -> str:
    normalized = _normalize_tmsh(command)
    if not normalized:
    ...
    if not lowered.startswith(READONLY_PREFIXES):
        raise ValueError(
            "Only tmsh show/list or ping/traceroute commands are allowed via read-only SSH"
        )
    padded = f" {lowered} "
    for token in _READONLY_BLOCKED:
        if token in padded:
...
def classify_f5_command(command: str) -> str:
    ...
    if not lowered:
        return "read"
    if lowered.startswith(READONLY_PREFIXES) or lowered.startswith(("show ", "list ", "ping ", "traceroute ")):
        return "read"
...
async def test_connection(host: str, username: str, password: str) -> tuple[bool, str]:
    ...
    if result.get("success"):
        version_line = (result.get("output") or "").splitlines()[0:5]
        snippet = " ".join(line.strip() for line in version_line if line.strip())
        return True, snippet or "SSH login successful"
    message = result.get("errorMessage") or result.get("stderr") or "tmsh show sys version failed"
...
async def run_tmsh_command(host: str, username: str, password: str, command: str) -> dict[str, Any]:
    return await _run_command(host, username, password, command, allow_writes=True, port=22, timeout=30.0)


async def run_tmsh_commands(host: str, username: str, password: str, commands: list[str]) -> dict[str, Any]:
    if not commands:
        raise ValueError("commands is required")
    results: list[dict[str, Any]] = []
    for command in commands:
        result = await run_tmsh_command(host, username, password, command)
